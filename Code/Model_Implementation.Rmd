...
Fig_3_comb_plot = ggplot()  + geom_point(data = Fig_3_combined_data, aes(x = var_value, y = LL, color = Metric, shape = Metric)) +
  ...
  geom_line(data = Fig_3_ABC_plot_data, aes(x = var_value, y = LL), color = 'black', size = 1.0) +
  scale_x_continuous(breaks = scales::pretty_breaks(n = 3)) +
  theme(
    aspect.ratio = 1,
    strip.background = element_blank(),
...
Fig_3_D = Fig_3_D + rahul_big_panel_theme
Fig_3_Panel_D = Fig_3_Panel_D + rahul_big_panel_theme
test = test + rahul_big_panel_theme
tiff(
  paste0(
    "../Figures/Manuscript_Figures/TIFF_Files/Fig3_raw.tiff"),
  height = 5, width = 10, res = 600, units = "in")# print(grid.arrange(Sup_Fig_3_A_comb, Sup_Fig_3_B_comb, legend,Sup_Fig_3_C_comb, Sup_Fig_3_D_comb,
#                    ncol = 3, widths = c(1,1,0.9)))
grid.arrange(grobs = list(Fig_3_ABC, test), ncol = 2, widths = c(3,1),
             heights = c(2.5,.25))
dev.off()
library(cowplot)
plot_grid(Fig_3_ABC, test,align = "h", ncol = 2, rel_widths = c(3/4, 1/4))
Fig_3_ABC
Fig_3_ABC_pan = Fig_3_ABC+ theme(strip.text = element_text(size = rel(3.0), vjust = -4.0), 
...
Fig_4_combined = Fig_4_combined + rahul_big_panel_theme +
  theme(strip.text = element_blank()) + theme(legend.position = c(.90, .55)) +
  guides(fill=FALSE, alpha = FALSE) +theme(axis.text.y = element_text(size = 18)) +
