...
# Simulate from test parameters
Note that in this model  we start the
initial test sim values for $\rho$ at $\rho = 0.03$ to match the literature.
...
old_I_0 = 79687.13
old_E_0 = 107813.5
new_I_0 = old_I_0 + old_E_0
new_duration_of_infection = (1/old_mu_EI) + (1/old_gamma_value)
new_gamma_value = 1/new_duration_of_infection
old_ref_model_name = "A_6"

  Top_2_LL_R_0_data = read.csv("../Generated_Data/Profiles/A_6_Model/A_6_Model_BP_top_2_LL_all_params_sim_R0_data.csv")
  head(Top_2_LL_R_0_data)
  
old_R_naught_MLE_values = read.csv(paste0("../Generated_Data/Profiles/",
                                          old_ref_model_name, "_Model/",
                                          old_ref_model_name, "_MLE_R_0_values.csv"))
old_R_naught_max = max(old_R_naught_MLE_values$R_0)
old_R_naught_min = min(old_R_naught_MLE_values$R_0)
one_year_old_R_naught_data = old_R_naught_MLE_values[5:16,]
old_R_naught_mean = mean(one_year_old_R_naught_data$R_0)
old_R_naught_amplitude = (old_R_naught_max - old_R_naught_min)/2
old_R_naught_mean
old_R_naught_amplitude
Beta_0_initial = old_R_naught_mean*(new_gamma_value +  3.68e-05)
delta_initial = (old_R_naught_amplitude*(new_gamma_value +  3.68e-05))/Beta_0_initial
Beta_0_max_guess = 2.0*(new_gamma_value +  3.68e-05)
omega_fixed = 2*pi/365
omega_fixed
params = c(Beta_0 = Beta_0_initial,
           delta =delta_initial, omega = omega_fixed, phi = pi/2,
           mu_H = 3.68e-05, gamma = new_gamma_value, N_0 = 5281842, rho = 0.02965919, sigma_P = 0.00019, sigma_M = 0.01,I_0 = new_I_0, R_0 = 0, C_0 = 0, r= 0, epsilon = 0)
true_sim_params = params
output_file_name = paste0("../Generated_Data/MIF_local_test_results/true_sim_params_mif_sim_test_", model_name, "_data_DENV1.RData")
save(true_sim_params,
     file = output_file_name)
time_seq = covar@times
covar_table = as.data.frame(t(covar@table))
Beta_t = params[1]*(1 + params[2]*sin(params[3]*covar@times + params[4]));

spline_plot_data = covar_table
spline_plot_data$time = covar_times

spline_plot_data = melt(spline_plot_data, id.vars = "time")
head(spline_plot_data)
p = ggplot(data = spline_plot_data, aes(x = time, y = value, color = variable)) + geom_point() + geom_line() +
  rahul_theme
p
#Beta_t
...
gamma = params[6]
mu_H = params[5]
R_0 = Beta_t/(gamma + mu_H)
plot_data = data.frame(time = time_seq,
                       R_0 = R_0)
p = ggplot(data = plot_data,
           aes(x = time, y= R_0)) + geom_point() + geom_line() +
  rahul_theme + xlab("Time (Days since Jan 1 1986)") + ylab(expression(paste(R[0])))
p
params
```
...
p = ggplot(data = comp_data,
           aes(x = time/365, y = value))+
  geom_line(aes(group = variable, color = variable)) + geom_point(aes(color = variable)) + rahul_theme + xlab("Years since Jan 1 1986")+ ylab("Y")
p



p = ggplot(data = comp_data,
           aes(x = time/365, y = log(value)))+
  geom_line(aes(group = variable, color = variable)) + geom_point(aes(color = variable)) + rahul_theme + xlab("Years since Jan 1 1986") + ylab("log(Y)")
p
...
p
```
##Plot simulation mean and standard deviations

```{r}

sim_data_mean_Y = aggregate(Y ~ time, sim_data, mean)
sim_data_sd = aggregate(Y ~ time, sim_data, sd)
sim_data_mean_plus_2_sd = sim_data_mean_Y$Y + 2*sim_data_sd$Y
sim_data_mean_minus_2_sd = sim_data_mean_Y$Y - 2*sim_data_sd$Y

comp_data = data.frame(time = sim_data_mean_Y$time,
                       sim_data_mean = sim_data_mean_Y$Y,
                      sim_data_mean_plus_2_sd = sim_data_mean_plus_2_sd,
                      sim_data_mean_minus_2_sd = sim_data_mean_minus_2_sd,
                       true_data = Rio_data_clean$Y)
comp_data = melt(comp_data, id.vars = c("time"))
...
comp_data_melt = melt(comp_data, id.vars = c("time", "sim_data_mean_plus_2_sd",
                                             "sim_data_mean_minus_2_sd"))




p = ggplot(data = comp_data_melt) +
  geom_ribbon(aes(x = time/365, ymin = sim_data_mean_minus_2_sd,
                  ymax = sim_data_mean_plus_2_sd), fill = "grey70") +
  geom_line(aes(x = time/365, y = value, color = variable)) +
  geom_point(aes(x = time/365, y = value, color = variable)) +
  rahul_theme +
  theme_white_background +
  ...
  ylab("Observed Monthly Cases")
p
file_name = paste0("../Figures/MIF_local_test_results/good_sim_params_DENV1_cases_vs_time_", model_name,
                   "_starting_sim_values_same_as_DENV4_start_sim.png")
png(file_name)
print(p)
dev.off()

p = ggplot(data = comp_data_melt) +
  geom_ribbon(aes(x = time/365, ymin = log(sim_data_mean_minus_2_sd),
                  ymax = log(sim_data_mean_plus_2_sd)), fill = "grey70") +
  geom_line(aes(x = time/365, y = log(value), color = variable)) +
  geom_point(aes(x = time/365, y = log(value), color = variable)) +
  rahul_theme +
  theme_white_background +
  ...
  ylab("log(Observed Monthly Cases)")
p
file_name = paste0("../Figures/MIF_local_test_results/good_sim_params_DENV1_log_cases_vs_time_", model_name,
                   "_starting_sim_values_same_as_DENV4_start_sim.png")
png(file_name)
print(p)
dev.off()
...
## Track Susceptible Depletion
```{r}
sim_data$S_over_N = sim_data$S/sim_data$N
sim_data_median = aggregate(S_over_N ~ time, sim_data, median)

sim_data_quant = aggregate(S_over_N ~ time, sim_data, quantile, probs = c(0.025, 0.975))

sim_data_quant$S_over_N = as.data.frame(sim_data_quant$S_over_N)

colnames(sim_data_quant$S_over_N) = c("Q2.5", "Q97.5")

comp_data = data.frame(time = sim_data_median$time,
                       sim_data_median = sim_data_median$S_over_N,
                      sim_data_low_Q = sim_data_quant$S_over_N$Q2.5,
                      sim_data_high_Q = sim_data_quant$S_over_N$Q97.5)
comp_melted_data = melt(comp_data, id.vars = c("time"))


p = ggplot(data = comp_melted_data,
           aes(x = time/365, y = value))+
  geom_line(aes(group = variable, color = variable)) + geom_point(aes(color = variable)) + rahul_theme + xlab("Years since Jan 1 1986") + ylab(expression(paste(frac(S,N))))
p
...
time_of_year = sim_data_median$time %% 365
R_0_sesonal_df = dplyr::select(plot_data, time_of_year = time, R_0 = R_0)
sim_time_and_time_of_year = data.frame(sim_data_median$time, time_of_year)
R_0_df = join(sim_time_and_time_of_year, R_0_sesonal_df)
R_0_df = dplyr::select(R_0_df, time = sim_data_median.time, R_0)
p  = ggplot(data = R_0_df, aes(x = time/365, y = R_0)) + geom_point() + geom_line() + rahul_theme + xlab("Years since Jan 1 1986") +
...
R_eff_melt_data = melt(R_eff_data, id.vars = c("time"))
p = ggplot(data = R_eff_melt_data, aes(x = time/365 , y = value)) +
  geom_line(aes(group = variable, color = variable)) + geom_point(aes(color = variable)) + rahul_theme + xlab("Years since Jan 1 1986") + ylab(expression(paste(R[eff])))
p


R_eff_melt_data = melt(R_eff_data, id.vars = c("time", "sim_low_Q",
                                               "sim_high_Q"))
p = ggplot(data = R_eff_melt_data)+
...
## Track Susceptible Depletion: mean and Standard Deviation
```{r}
sim_data_mean = aggregate(S_over_N ~ time, sim_data, mean)
sim_data_sd = aggregate(S_over_N ~ time, sim_data, sd)
sim_data_mean_plus_2_sd =sim_data_mean$S_over_N + 2*sim_data_sd$S_over_N
sim_data_mean_minus_2_sd =sim_data_mean$S_over_N - 2*sim_data_sd$S_over_N

comp_data = data.frame(time = sim_data_mean$time,
                       sim_data_mean = sim_data_mean$S_over_N,
                      sim_data_mean_plus_2_sd = sim_data_mean_plus_2_sd,
                      sim_data_mean_minus_2_sd = sim_data_mean_minus_2_sd)
comp_melted_data = melt(comp_data, id.vars = c("time"))
...
comp_melted_data = melt(comp_data, id.vars = c("time", "sim_data_mean_plus_2_sd",
                                               "sim_data_mean_minus_2_sd"))
...
time_of_year = sim_data_mean$time %% 365
R_0_sesonal_df = dplyr::select(plot_data, time_of_year = time, R_0 = R_0)
sim_time_and_time_of_year = data.frame(sim_data_mean$time, time_of_year)
R_0_df = join(sim_time_and_time_of_year, R_0_sesonal_df)
R_0_df = dplyr::select(R_0_df, time = sim_data_mean.time, R_0)
p  = ggplot(data = R_0_df, aes(x = time/365, y = R_0)) + geom_point() + geom_line() + rahul_theme + xlab("Years since Jan 1 1986") +
  ylab(expression(paste(R[0])))
p
R_0_with_S_df = join(R_0_df, comp_data)
R_0_with_S_df$R_eff_mean = R_0_with_S_df$R_0*R_0_with_S_df$sim_data_mean
R_0_with_S_df$R_eff_mean_plus_2_sd_S = R_0_with_S_df$R_0*R_0_with_S_df$sim_data_mean_plus_2_sd
R_0_with_S_df$R_eff_mean_minus_2_sd_S = R_0_with_S_df$R_0*R_0_with_S_df$sim_data_mean_minus_2_sd
#head(R_0_with_S_df)
R_eff_data = dplyr::select(R_0_with_S_df, time, sim_mean = R_eff_mean, sim_mean_plus_2_sd_S = R_eff_mean_plus_2_sd_S, sim_mean_minus_2_sd_S = R_eff_mean_minus_2_sd_S)
R_eff_melt_data = melt(R_eff_data, id.vars = c("time"))
p = ggplot(data = R_eff_melt_data, aes(x = time/365 , y = value)) +
  geom_line(aes(group = variable, color = variable)) + geom_point(aes(color = variable)) + rahul_theme + xlab("Years since Jan 1 1986") + ylab(expression(paste(R[eff]))) +
  theme_white_background
...
R_eff_melt_data = melt(R_eff_data, id.vars = c("time", "sim_mean_plus_2_sd_S",
                                               "sim_mean_minus_2_sd_S"))

p = ggplot(data = R_eff_melt_data)+
  geom_ribbon(aes(x = time/365, ymin = sim_mean_minus_2_sd_S,
                  ymax = sim_mean_plus_2_sd_S), fill = 'grey70') +
  geom_line(aes(x = time/365, y = value, color = variable))  +
  geom_point(aes(x = time/365, y = value,color = variable)) +
  rahul_theme + xlab("Years since Jan 1 1986") +  ylab(expression(paste(R[eff]))) +
  theme_white_background +
...
p = ggplot(data = pfilter_sim_data,(aes(x = time/365, y = Y))) + geom_point() + geom_line() + rahul_theme + xlab("Years since Jan 1 1986")
p
ptm <- proc.time()
...
```{r}
ptm <- proc.time()
pfilter_real_output = pfilter(data = Rio_data_clean,
                             seed = 12345,
                    times = Rio_data_clean$times,
                    t0 = t0,
                    rprocess = euler(rproc,delta.t = 1),
...
mif_sim_file_name = paste0("../Generated_Data/MIF_local_test_results/mif_sim_test_data_", model_name, "_DENV1.RData")
save(mif_sim_data,
...
p  = ggplot(data = mif_sim_data, aes(x = time/365, y = Y)) + geom_point() + geom_line() + xlab("Time (years since Jan 1 1986)") +
  rahul_theme
p
...
bake(file = rds_file_name,{
  foreach(i=1:10, .packages = 'pomp2',
          .export = c("rproc", "rmeas", "dmeas", "mif_sim_data", "init", "paramnames", "statenames", "obsnames",
                      "params", "par_trans", "acumvarnames", "covar")
  ) %dopar% {
...
results
result_output_file = paste0("Rio_city_DENV1_", model_name, "_sim_data_fit_params_dem_binom_with_env_stoch.csv")
...
test_1_mif = mif2(
              ...
              rw.sd = rw.sd(Beta_0 = ifelse( time <= 365*2.50, 0.02, 0),
                            delta = ifelse( time <= 365*2.50, 0.02, 0),
                            phi = ifelse( time <= 365*2.50, 0.02, 0),
                            rho = ifelse( time <= 365*2.50, 0.02, 0),
                            sigma_P = 0,
                            sigma_M = 0.02,
                            I_0 = ivp(0.2), R_0 = 0,
                            epsilon = 0)
...
Initially, we use a default random walk standard deviation  of $rw.sd = 0.02$  for all varied parameters ($rho$, $I_{0}$, $\beta_0$, $\delta$, $\phi$, and $\sigma_M$).  The random walk standard deviation for $\sigma_P$ is set to $rw.sd = 0$ as was the case for Model A_5.
All other parameters are assumed fixed for now. We use a cooling fraction of $cooling.fraction.50=0.5$.
We use the default POMP2 MIF2 settings for cooling type (geometric) and transforms (included).
```{r, echo = TRUE}
ptm = proc.time()
registerDoRNG(123456)
rds_index = rds_index + 1
...
#-114.8501394    0.0382844
# This is not higher than the  true sim mean of    -113.88982088 (se 0.04726053), so did a longer run.
```
...
ptm = proc.time()
rds_index = rds_index + 1
mif_sim_200_run_output_rds_file_name = paste0("Stew_Files/", model_name, "_", "rds_", rds_index, ".rds")
bake(file = mif_sim_200_run_output_rds_file_name,{
 mif2(
              data = mif_sim_data,
              times = mif_sim_data$time,
              t0 = t0,
...
MIF_sim_200_run_output_plot_file_name = paste0("../Figures/MIF_local_test_results/",
                                               model_name, "_test_mif_convergence_dem_stoch_plot_1_run_200_iterations_from_sim_data_truth.png")
png(MIF_sim_200_run_output_plot_file_name)
print(p)
dev.off()
...
ptm <- proc.time()
rds_index = rds_index + 1
pfilter_long_mif_sim_200_iterations_end_output_rds_file = paste0("Stew_Files/", model_name, "_", "rds_", rds_index, ".rds")
bake(file = pfilter_long_mif_sim_200_iterations_end_output_rds_file,{
  foreach(i=1:10, .packages = 'pomp2',
          .export = c("rproc", "rmeas", "dmeas", "mif_sim_data", "init", "paramnames", "statenames", "obsnames",
                      "params", "par_trans", "acumvarnames", "covar")
  ) %dopar% {
...
pfilter_long_mif_sim_200_iterations_end_LL_file_name = paste0("../Generated_Data/MIF_local_test_results/", model_name, "_validation_pfilter_ranges_200_iteration_single_MIF_run.RData")
save(L_pfilter_sim_at_long_mif_end_2,
     file = pfilter_long_mif_sim_200_iterations_end_LL_file_name)
#-111.36292964  0.03243299
# This is higher than true sim mean of  -113.88982088 (se 0.04726053), so it seems to have worked (at least, this is enough to move on to a grid search using Midway).
```
...
knitr::read_chunk('generate_profile_combinations_SIR_Spline.R')
```
...
knitr::read_chunk('MIF_run_Model_A_6.R')
```
...
#combine_profile_output(profile_var = "epsilon", model_name = model_name)

#combine_profile_output(profile_var = "N_0", model_name = model_name)
...
p = ggplot(data = profile_data_clean, aes_string(x = eval(profile_var), y = "LL")) + geom_point() + geom_hline(yintercept = cutoff_thres_20_LL_from_ML,
                                                                                                               color = 'red')+
  rahul_theme
print(p)
...
p = ggplot(data = profile_var_profile, aes_string(x = eval(profile_var), y = "LL")) +
...
pdf("../Figures/Manuscript_Figures/Figure_4/Man_Fig_3_v3.pdf")
# print(grid.arrange(Sup_Fig_3_A_comb, Sup_Fig_3_B_comb, legend,Sup_Fig_3_C_comb, Sup_Fig_3_D_comb,
#                    ncol = 3, widths = c(1,1,0.9)))
print(grid.arrange(Sup_Fig_3_A_comb, Sup_Fig_3_B_comb, Sup_Fig_3_C_comb,legend,
                   ncol = 2, widths = c(1,1)))
dev.off()
...
bio_good_2_LL = read.csv(paste0("../Generated_Data/Profiles/", model_name, "_Model/",model_name, "_Model_BP_top_2_LL_all_params_bio_good_2_LL_sim_4_years.csv"))
...
#Get 4% rho skip data
skip_data_rho_four_percent = 
  nCritics[four_percent_rho_index,
           c(3,5,7,8,9),
           c(91:181)]
skip_df_rho_4 = as.data.frame(skip_data_rho_four_percent)
skip_df_rho_4 <- tibble::rownames_to_column(skip_df_rho_4, "delta")
strsplit(skip_df_rho_4$delta, split = "_")
colnames(skip_df_rho_4)
rownames(skip_df_rho_4)
library(stringr)
skip_df_rho_4$delta = str_split(skip_df_rho_4$delta,
                          pattern = "_", simplify = TRUE)[,2]
head(skip_df_rho_4)

skip_df_rho_4 = melt(skip_df_rho_4, id.vars = c("delta"))
skip_df_rho_4 = dplyr::select(skip_df_rho_4, delta, r0 = variable,
                        Num_Skips = value)
skip_df_rho_4$r0 = str_split(skip_df_rho_4$r0,
                          pattern = "_", simplify = TRUE)[,2]
skip_df_rho_4$rho = 0.04
skip_df_rho_4$rho_lab = "\rho~=~0.04" 
...
pdf("../Figures/Manuscript_Figures/Figure_3/Man_Fig_3_single_delta_07.pdf",
    height = 5, width = 10)
print(p)
dev.off()
...
for(param_index in seq(1, nrow(A_7_bio_good_2_LL))){
  load("../Down_Data/Skip_Data/nCritics_detailedRepRate_From2to5.Rdata")
...
pdf("../Figures/Manuscript_Figures/Figure_7/Figure_7_Cleaned_Skips_Could.pdf",
    width = 10, height = 10)
print(p)
dev.off()

pdf("../Figures/Manuscript_Figures/Figure_7/Figure_7_Cleaned_Skips_no_zoom.pdf")
print(p)
dev.off()
