...
# under the approximation of the paper L. Stone et al we have -->
# dS/dt approx lam*N
...
# ==> dY/dt = (1+delta*sen(wt+pi)) * [exp(-(lam-mu)*t)*Z0/N0 +
# (1-exp(-(lam-mu)*t))*m/(lam-mu)] -1

# when integral(dY/dt)_from{0}_to_{time_high_season} > 0
# we have an outbreak. 
# Then I will look for the critical value which corresponds to:
#   z0_c * (lam-mu) = m - [ (m-lam+mu)*pi*(2*n+1)-2*delta]/(aux*w),
# with aux defined as follows in the script.
...
# instead of dS/dt approx lam*N, now we add the other term
# --> dS/dt approx (lam*N - mu*S)
...
source("rahul_theme.R")
  ...
  Fig_1_Panel_A = ggplot(data = plot_df, aes(x = n, y = sc)) +
    ...
    ylab(expression(atop("Susceptibility threshold ",
                         paste(" for n skips to occur ", (S[c](n)))))) +
