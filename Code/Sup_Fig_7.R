...
Sup_Fig_7_df_colnames = c("I_0","LL" ,"Model",     
                           "Model_Name", "Profile_Var")
Sup_Fig_7_prof_peak_treshold_df_colnames = c("Profile_threshold", "Model", "Model_Name")
Sup_Fig_7_MLE_value_for_prof_var_df_colnames = c("MLE_value_for_prof_var", "Model", "Model_Name")
Sup_Fig_7_prof_peak_value_for_prof_var_df_colnames = c("prof_peak_value_for_prof_var", "Model", "Model_Name")
...
Sup_Fig_7_df = data.frame(matrix(nrow = 0,
                                  ncol = length(Sup_Fig_7_df_colnames)))
Sup_Fig_7_prof_peak_treshold_df = data.frame(matrix(nrow = 0,
                                                     ncol = length(Sup_Fig_7_prof_peak_treshold_df_colnames)))
Sup_Fig_7_MLE_value_for_prof_var_df =
  data.frame(matrix(nrow = 0, ncol = length(Sup_Fig_7_MLE_value_for_prof_var_df_colnames)))
Sup_Fig_7_prof_peak_value_for_prof_var_df =
  data.frame(matrix(nrow = 0, ncol = length(Sup_Fig_7_prof_peak_value_for_prof_var_df_colnames)))

ML_df = data.frame(matrix(nrow = 0,
                                  ncol = length(ML_df_colnames)))

colnames(Sup_Fig_7_df) = Sup_Fig_7_df_colnames
colnames(Sup_Fig_7_prof_peak_treshold_df) = Sup_Fig_7_prof_peak_treshold_df_colnames
colnames(Sup_Fig_7_MLE_value_for_prof_var_df) = Sup_Fig_7_MLE_value_for_prof_var_df_colnames
colnames(Sup_Fig_7_prof_peak_value_for_prof_var_df) = Sup_Fig_7_prof_peak_value_for_prof_var_df_colnames
...
for(model_index in seq(1:length(model_name_list))){
  ...
  Sup_Fig_7_df = rbind(Sup_Fig_7_df, single_model_output_list[[1]])
  Sup_Fig_7_prof_peak_treshold_df = rbind(Sup_Fig_7_prof_peak_treshold_df, single_model_output_list[[2]])
  Sup_Fig_7_MLE_value_for_prof_var_df = rbind(Sup_Fig_7_MLE_value_for_prof_var_df,
                                               single_model_output_list[[3]])
  Sup_Fig_7_prof_peak_value_for_prof_var_df = rbind(Sup_Fig_7_prof_peak_value_for_prof_var_df,
...
Sup_Fig_7_df = Sup_Fig_7_df %>%
  mutate(var_value = I_0) %>%
  dplyr::select(-I_0)

ymin = Sup_Fig_7_prof_peak_treshold_df %>%
  group_by(Model) %>%
  summarize(ymin = Profile_threshold-10) %>%
  as.data.frame() 

min_prof_value = Sup_Fig_7_df %>%
  group_by(Model) %>%
...
Sup_Fig_7_df_clean = filter(Sup_Fig_7_df, LL > y_lim_min)

plot_label_df = data.frame(Model_Name = Sup_Fig_7_MLE_value_for_prof_var_df$Model_Name,
                           plot_var_label = c("I_0", "I_0", "I_0"),
                           model_var_label = c("SIR Cosine", "SIR Spline", "SEIR Spline"))
Sup_Fig_7_df = join(Sup_Fig_7_df, plot_label_df)

Sup_Fig_7_ABC_plot_data = join(Sup_Fig_7_df, Sup_Fig_7_MLE_value_for_prof_var_df)


Sup_Fig_7_ABC_plot_data
head(Sup_Fig_7_ABC_plot_data)

Sup_Fig_7_ABC_plot_data = join(Sup_Fig_7_ABC_plot_data, ML_df)
Sup_Fig_7_ABC_plot_data = join(Sup_Fig_7_ABC_plot_data, Sup_Fig_7_prof_peak_treshold_df)

cutoff_value  = -174 
Sup_Fig_7_ABC_plot_data = filter(Sup_Fig_7_ABC_plot_data, LL > ML - 10 )

Sup_Fig_7_ABC_plot_data$Metric = "LL"
Sup_Fig_7_ABC_plot_data$low_bound = ML-11
Sup_Fig_7_ABC_plot_data$Line_Color = "Show_Line"

Sup_Fig_7_combined_data = Sup_Fig_7_ABC_plot_data
Sup_Fig_7_combined_data$Model_Name = factor(Sup_Fig_7_combined_data$Model_Name, levels = c("SIR Cosine",
                                                                                           "SIR Spline","SEIR Spline"))
head(Sup_Fig_7_combined_data)

#### I_0 Profile SIR Cosine Model
I_0_poly_data_SIR_Cosine = Sup_Fig_7_ABC_plot_data %>%
  filter(Profile_Var == "I_0") %>%
  filter(Model == "A_7") %>%
...
small_breaks_I_0_SIR_Cosine = seq(from= min(I_0_poly_data_SIR_Cosine$var_value),
                                  to = max(I_0_poly_data_SIR_Cosine$var_value), length = 10^3)
...
#### I_0 Profile SIR Spline Model
I_0_poly_data_SIR_Spline = Sup_Fig_7_ABC_plot_data %>%
  filter(Profile_Var == "I_0") %>%
  filter(Model == "A_6") %>%
  dplyr::select(Profile_Var, Model, var_value, LL)
...
small_breaks_I_0_SIR_Spline = seq(from= min(I_0_poly_data_SIR_Spline$var_value),
                                  to = max(I_0_poly_data_SIR_Spline$var_value), length = 10^3)
...
#### I_0 Profile SEIR Spline Model
I_0_poly_data_SEIR_Spline = Sup_Fig_7_ABC_plot_data %>%
  filter(Profile_Var == "I_0") %>%
  filter(Model == "A_5") %>%
  dplyr::select(Profile_Var, Model, var_value, LL)
...
small_breaks_I_0_SEIR_Spline = seq(from= min(I_0_poly_data_SEIR_Spline$var_value),
                                  to = max(I_0_poly_data_SEIR_Spline$var_value), length = 10^3)
...
combined_poly_data = rbind(I_0_SIR_Cosine_poly_curve_df, I_0_SIR_Spline_poly_curve_df)
combined_poly_data = rbind(combined_poly_data, I_0_SEIR_Spline_poly_curve_df)
...
### Plot Figure S7
Sup_Fig_7_combined_data$model_var_label = factor(Sup_Fig_7_combined_data$model_var_label, levels = c("SIR Cosine", "SIR Spline",
                                                       "SEIR Spline"))
Fig_S7_comb_plot = ggplot()  + geom_point(data = Sup_Fig_7_combined_data, aes(x = var_value, y = LL, color = Metric, shape = Metric)) +
  scale_linetype_manual(values = c("blank", "solid")) +
  rahul_man_figure_theme + rahul_big_panel_theme  +theme_white_background  +
  geom_hline(data = Sup_Fig_7_combined_data, aes(yintercept = Profile_threshold), size = 1.0, linetype = "dashed", color = 'grey70') +
  geom_vline(data = Sup_Fig_7_combined_data, aes(xintercept = MLE_value_for_prof_var),
             size = 1.0, linetype = "twodash",show.legend= F, color = 'grey70') +
  facet_wrap(~model_var_label, scales = "free", 
             ncol = 1) +
  geom_hline(data = Sup_Fig_7_combined_data, aes(yintercept = low_bound), color = 'white', linetype = 'blank') +
...
pdf(
  paste0(
    "../Figures/Supplemental_Figures/Supplemental_Figure_7/Supplemental_Figure_7.pdf"),
  height = 10, width = 5)
Fig_S7_comb_plot
dev.off()
